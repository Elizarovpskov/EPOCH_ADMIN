...
{
$pagetitle = "FASTTRAVEL";

//if ( $serverinstance == '11')
{
$FASTTRAVEL ='
...
$PLAYER     = mysql_real_escape_string($_POST['select_player']);
}

if (!empty($_POST['enter_player'])){
$PLAYER      = mysql_real_escape_string($_POST['enter_player']);
}
...
$TRAVEL   = mysql_real_escape_string($_POST['FASTTRAVEL']);
$P1 = mysql_query("SELECT `PlayerUID`, `PlayerName` FROM `player_data` WHERE `PlayerUID` = '".$PLAYER."' OR `PlayerName`= '".$PLAYER."'");
while ($row = mysql_fetch_object($P1)){
$PLAYER_UID   = $row->PlayerUID;
$PLAYER_NAME  = $row->PlayerName;
}

$P2 = mysql_query("SELECT `Worldspace` FROM `character_data` WHERE `PlayerUID` = '".$PLAYER_UID."' AND `InstanceID`='".$serverinstance."' AND `Alive`='1'");
if (mysql_num_rows($P2) == TRUE ) {
mysql_query("UPDATE `character_data` SET `Worldspace` = '[0,[".$TRAVEL."]]' WHERE `PlayerUID` = '".$PLAYER."' AND `InstanceID`='".$serverinstance."' AND `Alive`='1' ");
...
$select_iv  = mysql_real_escape_string($_POST['select_iv']);
$P1 = mysql_query("SELECT `PlayerUID`, `PlayerName` FROM `player_data` WHERE `PlayerUID` = '".$PLAYER."' OR `PlayerName`= '".$PLAYER."'");
while ($row = mysql_fetch_object($P1)){
$PLAYER_UID   = $row->PlayerUID;
$PLAYER_NAME  = $row->PlayerName;
...
$P2 = mysql_query("SELECT `Worldspace` FROM `character_data` WHERE `PlayerUID` = '".$PLAYER_UID."' AND `InstanceID`='".$serverinstance."' AND `Alive`='1'");
if (mysql_num_rows($P2) == TRUE ) {
mysql_query("UPDATE `character_data` SET `Worldspace` = '".$select_iv."' WHERE `PlayerUID` = '".$PLAYER."' AND `InstanceID`='".$serverinstance."' AND `Alive`='1' ");
...
$vehicle_result = mysql_query("SELECT `Classname`, `Worldspace` FROM `object_data` 
WHERE `Instance` = '".$serverinstance."' 
AND 
...
<option value="" /></option>';
while($row = mysql_fetch_array($vehicle_result)) {
