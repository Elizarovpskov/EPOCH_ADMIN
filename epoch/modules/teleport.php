...
{
$pagetitle = "Teleport player";
######## SUBMIT  #################################
if (isset($_POST['teleport'])) {


if (!empty($_POST['select_player2'])){
$PLAYER2 = mysql_real_escape_string($_POST['select_player2']);
}
if (!empty($_POST['enter_player2'])){
$PLAYER2 = mysql_real_escape_string($_POST['enter_player2']);
}
...
$profile = mysql_query("SELECT `PlayerUID`, `PlayerName` FROM `player_data` WHERE `PlayerUID` = '".$PLAYER2."' OR `PlayerName`= '".$PLAYER2."' ");
...
if (!empty($PLAYER2_ID)) {
$P2 = mysql_query("SELECT `Worldspace` FROM `character_data` WHERE `PlayerUID` = '".$PLAYER2_ID."' AND `InstanceID`='".$serverinstance."' AND `Alive`='1'");
if (mysql_num_rows($P2) == TRUE ) {
$row = mysql_fetch_object($P2);
...
$PLAYER1 = mysql_real_escape_string($_POST['select_player1']);
}
if (!empty($_POST['enter_player1'])){
$PLAYER1 = mysql_real_escape_string($_POST['enter_player1']);
}
...
$profile = mysql_query("SELECT `PlayerUID`, `PlayerName` FROM `player_data` WHERE `PlayerUID` = '".$PLAYER1."' OR `PlayerName`= '".$PLAYER1."' ");
if (mysql_num_rows($profile) == TRUE ) {
$row = mysql_fetch_object($profile);
...
$P1 = mysql_query("SELECT `Worldspace` FROM `character_data` WHERE `PlayerUID` = '".$PLAYER1_ID."' AND `InstanceID`='".$serverinstance."' AND `Alive`='1'");
if (mysql_num_rows($P1) == TRUE ) {

if(isset($PLAYER2_WORLDSPACE)){

mysql_query("UPDATE `character_data` SET `Worldspace` = '".$PLAYER2_WORLDSPACE."' WHERE `PlayerUID` = '".$PLAYER1_ID."' AND `InstanceID`='".$serverinstance."' AND `Alive`='1' ");
